...
/// Ephemeral UI state for the Orders list — just the active filter. No
/// `setState`, no logic in the View. Counts + slices are derived here so the
...
class OrdersViewController {
  ...
  /// Orders matching the active filter (done + failed both count as "done").
  List<FlowOrder> get visible => switch (filter.value) {
