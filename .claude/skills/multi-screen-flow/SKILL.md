...
│       ├──→ Create Screen               │                 │
│       │         │                       │                 │
│       │    Modular.to.pop(newEntity)      Modular.to.pop(updatedEntity)│
│       │         │                       │                 │
│       ◄─────────┘                       │                 │
│       ◄─────────────────────────────────┘                │
│                                                          │
...
│       ◄────────────────┘                                 │
│       │                                                   │
│  Modular.to.pop(true) → List removes item                       │
└──────────────────────────────────────────────────────────┘
```
...
class _ProductsBody extends StatelessWidget {
  ...
  Future<void> _goToDetail(BuildContext context, ProductEntity product) async {
    final wasDeleted = await Modular.to.pushNamed<bool>('/target', arguments: product: product);
    if (wasDeleted == true) {
...
Future<void> _goToCreate(BuildContext context) async {
  final newProduct = await Modular.to.pushNamed<ProductEntity>('/target', arguments: const CreateProductScreen());
...
class ProductDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        BlocProvider(create: (_) => Modular.get<ProductDetailCubit>()..fetchProduct(product.id)),
...
class _ProductDetailBody extends StatelessWidget {
  ...
  Future<void> _goToEdit(BuildContext context, ProductEntity product) async {
    final updated = await Modular.to.pushNamed<ProductEntity>('/target', arguments: product: product);
...
class CreateProductScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => Modular.get<CreateProductCubit>(),
      child: DefaultScaffold(
...
class EditProductScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => Modular.get<EditProductCubit>(),
      child: DefaultScaffold(
...
- [ ] `Modular.to.pop(result)` returns correct type
- [ ] `context.mounted` checked after every `await Modular.to.pushNamed()`
